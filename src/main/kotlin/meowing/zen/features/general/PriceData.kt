        ...
        register<ItemTooltipEvent> { event ->
            ...
            if (0 in displaySet) {
                pricingData.takeIf { it.has("activeBin") || it.has("activeAuc") }?.let {
                    ...
                    event.lines.add(Text.literal("§3Active Listings: §e${activeBin} §8[BIN] §7• §e${activeAuc} §8[Auction]"))
                }
            }
            ...
            if (1 in displaySet) {
                pricingData.takeIf { it.has("binSold") || it.has("aucSold") }?.let {
                    ...
                    event.lines.add(Text.literal("§3Daily Sales: §e${soldBin} §8[BIN] §7• §e${soldAuc} §8[Auction]"))
                }
            }
            ...
            if (2 in displaySet) {
                pricingData.takeIf { it.has("avgLowestBin") && it.has("lowestBin") }?.let {
                    ...
                    event.lines.add(Text.literal("§3BIN Price: §a${avgLowestBin} §8[Avg] §7• §a${lowestBin} §8[Lowest]"))
                }
            }
            ...
            if (3 in displaySet) {
                pricingData.takeIf { it.has("avgAucPrice") && it.has("aucPrice") }?.let {
                    ...
                    event.lines.add(Text.literal("§3Auction Price: §a${avgAucPrice} §8[Avg] §7• §a${aucPrice} §8[Next]"))
                }
            }
            ...
            if (4 in displaySet) {
                pricingData.takeIf { it.has("bazaarBuy") || it.has("bazaarSell") }?.let {
                    ...
                    val bazaarSell = it.takeIf { it.has("bazaarSell") }
                        ...
                        ?.times(multiplier)
                        ?.formatPrice() ?: "§7N/A"
                    event.lines.add(Text.literal("§3Bazaar: §e${bazaarBuy} §8[Buy] §7• §a${bazaarSell} §8[Sell]"))
                }
            }
        }
    }
