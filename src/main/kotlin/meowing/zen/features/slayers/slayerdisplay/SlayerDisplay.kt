...
object SlayerDisplay : Feature("slayerdisplay") {
    ...
    private var killTimers = mutableMapOf<Int, Triple<Long, String, Vec3d>>()
    ...
    override fun initialize() {
        register<EntityEvent.Metadata> { event ->
            ...
            slayerMobRegex.find(cleanName)?.let {
                val entityId = event.packet.id - 1
        ...
        register<TickEvent.Client> {
            ...
            entities.removeAll { entityId ->
                ...
                val nametag = world?.getEntityById(entityId + 1)?.name?.string?.removeFormatting() ?: return@removeAll true
                val timerNametag = world?.getEntityById(entityId + 2)?.name?.string?.removeFormatting() ?: ""
                val spawnerNametag = world?.getEntityById(entityId + 3)?.name?.string?.removeFormatting() ?: ""
                ...
                if ((hpMatch != null && 1 in displayOptions) || (hitsMatch != null && 2 in displayOptions)) {
                    ...
                    when {
                        hpMatch != null && 1 in displayOptions -> {
                            ...
                            val maxHp = (entity as LivingEntity).baseMaxHealth
        ...
        register<RenderEvent.EntityPost> { event ->
            ...
            displayData[event.entity.id]?.let { displayText ->
                Render3D.drawString(
                    displayText,
                    event.entity.pos.add(0.0, 0.25, 0.0),
                    scaleMultiplier = 1.5,
