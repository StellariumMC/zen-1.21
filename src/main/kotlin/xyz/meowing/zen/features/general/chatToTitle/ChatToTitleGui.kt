...
import xyz.meowing.zen.features.general.chatToTitle.ChatToTitle.removePattern
import xyz.meowing.zen.ui.Theme
import kotlin.math.max

class ChatToTitleGui : VexelScreen("Chat to Title") {
    private lateinit var listContainer: Container
    private lateinit var inputField: TextInput
    private lateinit var titleField: TextInput
    ...
    override fun onCloseGui() {
        super.onCloseGui()
        ChatToTitle.patternData.forceSave()
    }
    ...
    private fun createHeader(parent: Rectangle) {
        ...
        Text("Chat to Title", Theme.Text.color, 24f)
    ...
    private fun createFooter(parent: Rectangle) {
        ...
        inputField = TextInput("", "Enter pattern...", hoverColor = null)
            .setSizing(45f, Size.ParentPerc, 36f, Size.Pixels)
            .setPositioning(0f, Pos.ParentPixels, 0f, Pos.ParentCenter)
            .fontSize(13f)
            .backgroundColor(Theme.Bg.color)
            .borderColor(Theme.Border.color)
        ...
        titleField = TextInput("", "Enter title...", hoverColor = null)
            .setSizing(45f, Size.ParentPerc, 36f, Size.Pixels)
            .setPositioning(50f, Pos.ParentPercent, 0f, Pos.ParentCenter)
            .fontSize(13f)
            .backgroundColor(Theme.Bg.color)
            .borderColor(Theme.Border.color)
            .borderRadius(6f)
    ...
    private fun renderPatterns() {
        ...
        if (ChatToTitle.patterns.isEmpty()) {
        ...
        ChatToTitle.patterns.forEachIndexed { index, pattern ->
            createPatternRow(index, pattern)
        }
    }

    private fun createPatternRow(index: Int, pattern: ChatTitlePattern) {
        ...
        val textInput = TextInput(pattern.pattern, "Pattern", hoverColor = null)
            .setSizing(38f, Size.ParentPerc, 28f, Size.Pixels)
            .setPositioning(0f, Pos.ParentPixels, 0f, Pos.ParentCenter)
            .fontSize(12f)
        ...
        val titleInput = TextInput(pattern.title, "Title", hoverColor = null)
            .setSizing(38f, Size.ParentPerc, 28f, Size.Pixels)
            .setPositioning(41f, Pos.ParentPercent, 0f, Pos.ParentCenter)
            .fontSize(12f)
            .backgroundColor(Theme.BgLight.color)
            .borderColor(Theme.BorderMuted.color)
            .borderRadius(4f)
        ...
        textInput.onValueChange { text ->
            ChatToTitle.updatePattern(index, text as String, titleInput.value, pattern.filterType)
        }

        titleInput.onValueChange { text ->
            ChatToTitle.updatePattern(index, textInput.value, text as String, pattern.filterType)
        }

        val buttons = createFilterButtons(row, index, pattern)
        createDeleteButton(row, index)

        patternRows[index] = PatternRow(row, textInput, titleInput, buttons)
    }

    private fun createFilterButtons(parent: Rectangle, index: Int, pattern: ChatTitlePattern): List<Button> {
        ...
        val types = listOf(
            ChatToTitleType.CONTAINS to "Contains",
            ChatToTitleType.EQUALS to "Equals",
            ChatToTitleType.REGEX to "Regex"
        )
    ...
    private fun createDeleteButton(parent: Rectangle, index: Int) {
        Button("×", Theme.Danger.color, fontSize = 18f)
            ...
            .padding(4f)
            .onClick { _, _, _ ->
                if (removePattern(index)) renderPatterns()
                true
            }
            .apply {
                onHover(
    ...
    private fun updatePatternType(index: Int, filterType: ChatToTitleType) {
        val pattern = ChatToTitle.patterns.getOrNull(index) ?: return
        if (ChatToTitle.updatePattern(index, pattern.pattern, pattern.title, filterType)) {
            val row = patternRows[index] ?: return
            val types = listOf(ChatToTitleType.CONTAINS, ChatToTitleType.EQUALS, ChatToTitleType.REGEX)
