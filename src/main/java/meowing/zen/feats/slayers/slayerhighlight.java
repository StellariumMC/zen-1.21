...
public class slayerhighlight {
    public static void initialize() {
        WorldRenderEvents.AFTER_ENTITIES.register((context -> {
            if (Zen.getConfig().slayerhighlight || !isFighting || BossId == -1) return;
            ...
            Vec3d entityPos = bossEntity.getLerpedPos(context.tickCounter().getTickProgress(false));
            ...
            renderEntityBox(context.matrixStack(), Objects.requireNonNull(context.consumers()), x, y, z, width, height);
        }));
    }
    ...
    private static void renderEntityBox(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double x, double y, double z, float width, float height) {
